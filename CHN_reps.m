...
dup_ind=strfind(cell2mat(MVCO_CHN_reps(:,9))','b');
MVCO_CHN_reps(dup_ind,15:16)=MVCO_CHN_reps(dup_ind,17:18);
MVCO_CHN_reps = [MVCO_CHN_reps(:,1:14) MVCO_CHN_reps(:,16) MVCO_CHN_reps(:,19)]; %don't need colum15 chn_a because already have chn_name earlier in matrix, redundant 
header_chn(15)={'CHN_amt_filt'}; %get rid of sample a & b destinctions
header_chn(16)={'Notes'};
header_chn=header_chn(1:16); %get rid of sample b columns because those values are completely in "a" columns now

%not all entries in blanks excel sheet have been run. only keep rows that have data
%get rid of a few columns. just want date combusted, date run, N value, C value
blanks_orginaldata = blanks;
header_blanks_originaldata = header_blanks;
ind = find(~isnan(cell2mat(blanks(:,2)))); %only want blanks that have been run - column 2, run date
header_blanks = [header_blanks(1:2)' header_blanks(6:7)']; %condense header to the 4 columns
blanks = [cell2mat(blanks(ind,1:2)) cell2mat(blanks(ind,6:7))]; %just 4 columns, make doubles
blanks(:,1) = datenum(num2str(blanks(:,1)),'yyyymmdd');%matdate faster to work with
blanks(:,2) = datenum(num2str(blanks(:,2)),'yyyymmdd');
header_blanks(1) = {'Date Combusted, matdate'};
header_blanks(2) = {'Date Run, matdate'};
clear ind
