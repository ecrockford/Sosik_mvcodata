...
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
%
%5/6/2015 - Taylor - updated datadir after C: on Queenrose failed and had
%to rebuild computer using old Samwise C drive
%
%last updated 5/6/2014 - Taylor - updated datadir after switching from
%Samwise to Queenrose - tried using \\queenrose\J_data but fopen/fprintf
%wouldn't work, so had to use local address of J
commentstr='For submission to seabass.';
datadir = 'F:\from_Samwise\data\mvcodata\spectra\';%*see note directly above
investigators = 'Heidi_Sosik';
contact = 'hsosik@whoi.edu';
data_type = 'scan';
data_status = 'preliminary';
%doc & cal hardwired into script below ~line 233
    ...
    for rep = 1:2,
        ...
        if ~strcmp(apfilename,'-9999') | ~strcmp(acdomfilename,'-9999') | ~strcmp(adfilename,'-9999'),  %only make a seabass file if at least one file exists
            ...
            if ~strcmp(acdomfilename,'-9999'),
                [wvlncdom,acdom,abs_ag]=acdomspecproc_Dec2012(acdomfilename,acdomblankfilename,660,670); %edited by Taylor Dec 18, 2012
%above line edited to also output abs_ag because seabass wants absorbance (optical density) values now too
                goodcdom=find(wvlncdom>=300 & wvlncdom<=850);
            end;
            %process ap/ap if one exists
            if (~strcmp(apfilename,'-9999') | ~strcmp(adfilename,'-9999'))
                [wvln,ap,aph,ad,abs_ap,abs_ad]=apspecproc_Dec2012(apfilename,adfilename,volfilt*1e-6,apfiltarea*1e-4,780,800); %edited by Taylor Dec 18, 2012
%above line edited to also output abs_ag because seabass wants absorbance (optical density) values now too
            ...
            if exist('ap','var') & ~exist('acdom','var')
                ...
                abs_ag=repmat(-9999,length(ap),1);%added by Taylor Dec2012
                goodcdom=[1:length(good)]';
            elseif exist('acdom','var') & ~exist('ap','var')
                ap=repmat(-9999,length(acdom),1);
                abs_ap=repmat(-9999,length(acdom),1);%added by Taylor Dec2012
                ad=repmat(-9999,length(acdom),1);
                abs_ad=repmat(-9999,length(acdom),1);%added by Taylor Dec2012
            ...
            fprintf(fid,'/investigators=%-11s\n',investigators);
            fprintf(fid,'/experiment=%-4s\n',experiment);
            fprintf(fid,'/cruise=%-s\n',cruise);
            fprintf(fid,'/station=%-2s\n',station);
...
%            fprintf(fid,'/data_status=update\n');  %'final' is also an option
            ...
            fprintf(fid,'!\n');
            fprintf(fid,'! Raw File Names (Absorbance Files): %-s, %-s, %s\n',acdomfilename_simple,apfilename_simple,adfilename_simple);%taylor Dec2012 added "(Absorbance Files)"
            fprintf(fid,'!\n');
            if ~isnan(volfilt),
            ...
            if ~chlinc
                fprintf(fid,'/fields=wavelength,ag,ap,ad,aph,abs_ag,abs_ap,abs_ad\n');%edit by Taylor Dec2012-add abs_ag, abs_ap,abs_ad
                fprintf(fid,'/units=nm,1/m,1/m,1/m,1/m,unitless,unitless,unitless\n');%edit by Taylor Dec2012-add abs_ag, abs_ap,abs_ad
            else
                fprintf(fid,'/fields=wavelength,ag,ap,ad,aph,a*ph,abs_ag,abs_ap,abs_ad\n');%edit by Taylor Dec2012-add abs_ag, abs_ap,abs_ad
                fprintf(fid,'/units=nm,1/m,1/m,1/m,1/m,m^2/mg,unitless,unitless,unitless\n');%edit by Taylor Dec2012-add abs_ag, abs_ap,abs_ad
            end
            fprintf(fid,'/end_header\n');
            if ~chlinc
                for rep3=1:length(good)
                    fprintf(fid,'%-3.0f %-5.6f %-5.6f %-5.6f %-5.6f %-5.6f %-5.6f %-5.6f\n',wvln(good(rep3)),acdom(goodcdom(rep3)),ap(good(rep3)),ad(good(rep3)),aph(good(rep3)),abs_ag(good(rep3)),abs_ap(good(rep3)),abs_ad(good(rep3)));
                end
            else
                for rep3=1:length(good)
                    fprintf(fid,'%-3.0f %-5.6f %-5.6f %-5.6f %-5.6f %-5.6f %-5.6f %-5.6f %-5.6f\n',wvln(good(rep3)),acdom(goodcdom(rep3)),ap(good(rep3)),ad(good(rep3)),aph(good(rep3)),aphstar(good(rep3)),abs_ag(good(rep3)),abs_ap(good(rep3)),abs_ad(good(rep3)));
                end
            end
            fclose(fid);
%            save S_acdom440_values.mat slope440 all_acdom440 all_acdom443 all_acdom490 all_acdom412 matedate
            clear ap aph ad acdom wvln wvlncdom good goodcdom abs_*
